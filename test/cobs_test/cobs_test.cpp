#include <stdint.h>
#include <stdio.h>
#include <string.h>

static size_t byte_offset = 0;
static uint8_t *expected_output = NULL;
static bool did_fail = false;

// FIXME: actually use test harness

static inline void serial_write(uint8_t byte) {
    if (byte != expected_output[byte_offset]) {
        printf("\033[31m");
        did_fail = true;
    }
    printf("%02X \033[0m", byte);

    byte_offset++;
}

static inline void serial_write_cobs(const uint8_t *data, const size_t size) {
    if (data == NULL)
        return;

    const uint8_t *data_ptr = data;
    const uint8_t *end_ptr = data + size;

    while (data_ptr <= end_ptr) {
        // Distance to the next null byte or the end of the data
        size_t distance = strnlen((const char *)data_ptr, end_ptr - data_ptr);

        // Ensure the distance is clamped to 0xFF
        if (distance > 0xFE) {
            distance = 0xFE;
        }

        // Write the distance to the next null byte
        serial_write(distance + 1);
        // Write the non-null bytes
        for (int i = 0; i < distance; i++) {
            serial_write(*data_ptr);
            data_ptr++;
        }

        // Skip the next byte if the next null byte is < 0xFE bytes away
        // or if this is the last byte
        if (distance != 0xFE || data_ptr == end_ptr) {
            data_ptr++;
        }
    }

    serial_write(0);
}

#define TEST(NR)                                   \
    did_fail = false;                              \
    byte_offset = 0;                               \
    expected_output = output;                      \
    printf("\n----------TEST %d----------\n", NR); \
                                                   \
    printf("Input[%d]:\n", NR);                    \
    for (int i = 0; i < sizeof(input) - 4; i++)    \
        printf("%02X ", input[i]);                 \
                                                   \
    printf("\nExpected[%d]:\n", NR);               \
    for (int i = 0; i < sizeof(output); i++)       \
        printf("%02X ", output[i]);                \
                                                   \
    printf("\nActual[%d]:\n", NR);                 \
    serial_write_cobs(input, sizeof(input) - 4);   \
                                                   \
    printf("\n----------");                        \
                                                   \
    if (did_fail) {                                \
        printf("\033[31mFAIL\033[0m");             \
    } else {                                       \
        printf("\033[32mPASS\033[0m");             \
    }                                              \
                                                   \
    printf("----------\n");

inline static void test1(void) {
    uint8_t input[] = {
        0x00,

        0xDE, 0xAD, 0xBE, 0xEF
    };
    uint8_t output[] = { 0x01, 0x01, 0x00 };

    TEST(1)
}

inline static void test2(void) {
    uint8_t input[] = {
        0x00, 0x00,

        0xDE, 0xAD, 0xBE, 0xEF
    };
    uint8_t output[] = { 0x01, 0x01, 0x01, 0x00 };

    TEST(2)
}

inline static void test3(void) {
    uint8_t input[] = {
        0x00, 0x11, 0x00,

        0xDE, 0xAD, 0xBE, 0xEF
    };
    uint8_t output[] = { 0x01, 0x02, 0x11, 0x01, 0x00 };

    TEST(3)
}

inline static void test4(void) {
    uint8_t input[] = {
        0x11, 0x22, 0x00, 0x33,

        0xDE, 0xAD, 0xBE, 0xEF
    };
    uint8_t output[] = { 0x03, 0x11, 0x22, 0x02, 0x33, 0x00 };

    TEST(4)
}

inline static void test5(void) {
    uint8_t input[] = {
        0x11, 0x22, 0x33, 0x44,

        0xDE, 0xAD, 0xBE, 0xEF
    };
    uint8_t output[] = { 0x05, 0x11, 0x22, 0x33, 0x44, 0x00 };

    TEST(5)
}

inline static void test6(void) {
    uint8_t input[] = {
        0x11, 0x00, 0x00, 0x00,

        0xDE, 0xAD, 0xBE, 0xEF
    };
    uint8_t output[] = { 0x02, 0x11, 0x01, 0x01, 0x01, 0x00 };

    TEST(6)
}

inline static void test7(void) {
    uint8_t input[] = {
        
    };
    uint8_t output[] = {
        
    };

    TEST(7)
}

inline static void test8(void) {
    uint8_t input[] = {
        
    };
    uint8_t output[] = {
        
    };

    TEST(8)
}

inline static void test9(void) {
    uint8_t input[] = {
        
    };
    uint8_t output[] = {
        
    };

    TEST(9)
}

inline static void test10(void) {
    uint8_t input[] = {
        
    };
    uint8_t output[] = {
        
    };

    TEST(10)
}

inline static void test11(void) {
    uint8_t input[] = {
        
    };
    uint8_t output[] = {
        
    };

    TEST(11)
}

int main(void) {
    // Tests pulled from Wikipedia
    // <https://en.wikipedia.org/wiki/Consistent_Overhead_Byte_Stuffing>
    test1();
    test2();
    test3();
    test4();
    test5();
    test6();
    test7();
    test8();
    test9();
    test10();
    test11();
}
